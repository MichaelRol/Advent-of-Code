		...
		for (String cmd : wire1str) {
			if (cmd.charAt(0) == 'U') {
				for (int i = 0; i < Integer.parseInt(cmd.substring(1, cmd.length())); i++) {
					wire1curr[1]++;
					wire1.put(wire1curr[0] + " " + wire1curr[1], Math.abs(wire1curr[0]) + Math.abs(wire1curr[1]));
				}
			...
			if (cmd.charAt(0) == 'U') {
				for (int i = 0; i < Integer.parseInt(cmd.substring(1, cmd.length())); i++) {
					...
					if (wire1.containsKey(wire2curr[0] + " " + wire2curr[1])) {
						if (wire1.get(wire2curr[0] + " " + wire2curr[1]) < min) {
							min = wire1.get(wire2curr[0] + " " + wire2curr[1]);
						}
					}
				}
			} else if (cmd.charAt(0) == 'D') {
