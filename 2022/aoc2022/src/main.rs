...
fn run(day: &str, part: &str) {
    ...
    match day {
        ...
        "19" => {
            if part == "1" {
                now = Instant::now();
                result = puzzles::day_19::part1(get_path(day)).to_string();
                elapsed = now.elapsed();
            } else if part == "2" {
                let now = Instant::now();
                result = puzzles::day_19::part2(get_path(day)).to_string();
                elapsed = now.elapsed();
            } else {
                print!("Part number must be 1 or 2, but was {part}.\n");
                return;
...
fn run_all() {
    ...
    print!("Day 19 Part 1: {}\n", puzzles::day_19::part1(get_path("19")));
    print!("Day 19 Part 2: {}\n", puzzles::day_19::part2(get_path("19")));
